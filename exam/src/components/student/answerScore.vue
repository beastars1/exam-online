<template>
  <div class="score">
    <div class="title">
      <p class="name">{{ examData.course }}</p>
      <p class="description">（总分：{{ examData.fullScore }} 分,限时：{{ examData.totalTime }} 分钟）</p>
      <p class="description">学生：{{ studentData.name }}</p>
    </div>
    <div class="total">
      <div class="look">
        本次考试成绩
...
<!--        <div class="img1" :class="{'img1Transform': imgShow}">
        ...
        <div class="number" :class="{'border': isTransition}" v-if="score >= 0">
          <span>{{ score }}</span>
          <span>分数</span>
        </div>
        <div class="number" :class="{'border': isTransition}" v-else>
          <span>作弊</span>
        </div>
  ...
  methods: {
    ...
    getStudentInfo() {
      ...
      this.$axios(`/api/student/${studentId}`)
        .then(res => {
          this.studentData = {...res.data} //获取考试详情
        })
    },
    getExamInfo() {
